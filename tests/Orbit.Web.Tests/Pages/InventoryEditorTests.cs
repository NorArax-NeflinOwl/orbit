...
/// </summary>
public sealed class InventoryEditorTests : OrbitTestContext
{
    private static readonly Guid InventoryId = Guid.NewGuid();

    public InventoryEditorTests() => Services.AddSingleton(typeof(ILogger<>), typeof(NullLogger<>));
    ...
    public void An_item_dragged_onto_another_takes_its_place()
    {
        ...
        // arranging them here is what the inventory is read back in.
    ...
    /// <summary>
    /// An inventory being made rather than one being read - "/inventory/new" rather than
    /// "/inventory/{id}/edit". Nothing here exists yet, so nothing that depends on it doing so is
    /// offered - see InventoryEditor's own "!IsNewInventory" sections.
    /// </summary>
    [Fact]
    public void A_new_inventory_offers_nothing_that_needs_one_to_exist_first()
    {
        RegisterApiClientsForANewInventory();

        var cut = RenderComponent<InventoryEditor>();

        Assert.Contains("New inventory", cut.Find("h1").TextContent);
    ...
    [Fact]
    public void A_new_inventory_cannot_be_saved_before_it_has_a_name()
    {
        RegisterApiClientsForANewInventory();
        var cut = RenderComponent<InventoryEditor>();
    ...
    [Fact]
    public void Saving_a_new_inventory_creates_it_and_returns_to_the_list()
    {
        RegisterApiClientsForANewInventory();
    ...
    /// WriteFirstLine.
    /// </summary>
    private static void WriteTheName(IRenderedComponent<InventoryEditor> cut, string name)
    {
