...
/// <summary>
/// What an inventory says about itself beyond its name, and whether that survives a save. A field the
...
/// </summary>
public sealed class InventoryDescriptionTests : OrbitTestContext
{
    private static readonly Guid InventoryId = Guid.NewGuid();

    private string? _lastSavedJson;

    public InventoryDescriptionTests() => Services.AddSingleton(typeof(ILogger<>), typeof(NullLogger<>));
    ...
    [Fact]
    public void What_the_inventory_is_for_is_shown_under_its_name()
    {
    ...
    private void RegisterApiClients(string description, IReadOnlyList<InventoryItemDto>? items = null)
    {
        var handler = new StubHttpMessageHandler(request =>
        {
            ...
            if (request.Method == HttpMethod.Put && path.EndsWith($"/{InventoryId}", StringComparison.Ordinal))
            {
