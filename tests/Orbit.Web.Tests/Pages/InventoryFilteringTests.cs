...
/// </summary>
public sealed class InventoryFilteringTests : OrbitTestContext
{
    ...
    public InventoryFilteringTests() => Services.AddSingleton(typeof(ILogger<>), typeof(NullLogger<>));
    ...
    {
        // An empty list under a filter reads as an empty inventory, which is the one thing it is not.
        RegisterApiClients([
            Item("Flour", productType: "Food", category: "Dry"),
            Item("Bleach", productType: "Cleaning", category: "Under the sink")]);
        var cut = Render();
    ...
    {
        // The one thing a filter must never do. It hides rows from the screen, not from the inventory.
        RegisterApiClients([
            Item("Flour", productType: "Food", category: "Dry"),
            Item("Bleach", productType: "Cleaning", category: "Under the sink")]);
        var cut = Render();
    ...
    {
        // A private inventory sealed before units existed: its rows come back with none. The picker used
    ...
    {
        // The same rule the filters follow: it hides rows from the screen, not from the inventory.
        RegisterApiClients([Item("Flour"), Item("Sugar")]);
        var cut = Render();
        SearchFor(cut, "flour");
    ...
    private static void SearchFor(IRenderedComponent<InventoryEditor> cut, string name)
    ...
    private static void MoveRow(IRenderedComponent<InventoryEditor> cut, int row, string which)
        => ButtonOn(cut, row, which).Click();

    private static AngleSharp.Dom.IElement ButtonOn(IRenderedComponent<InventoryEditor> cut, int row, string which)
    ...
    private IRenderedComponent<InventoryEditor> Render()
        => RenderComponent<InventoryEditor>(parameters => parameters.Add(editor => editor.InventoryId, InventoryId));

    private static void ClickButtonSaying(IRenderedComponent<InventoryEditor> cut, string label)
    ...
    private static void ChooseProductType(IRenderedComponent<InventoryEditor> cut, string productType)
        => cut.FindAll(".inventory-filters select").First().Change(productType);

    private static void ChooseCategory(IRenderedComponent<InventoryEditor> cut, string category)
