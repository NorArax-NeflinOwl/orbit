...
public sealed class DeleteTaskListCommandHandlerTests
{
    ...
    public async Task HandleAsync_deletes_a_task_list_owned_by_the_requesting_user()
    {
        ...
        var handler = new DeleteTaskListCommandHandler(repository, new InMemoryTaskListShareRepository());
    ...
    {
        var handler = new DeleteTaskListCommandHandler(new InMemoryTaskRepository(), new InMemoryTaskListShareRepository());
