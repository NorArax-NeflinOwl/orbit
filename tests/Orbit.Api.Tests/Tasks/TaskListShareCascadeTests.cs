...
public sealed class TaskListShareCascadeTests
{
    ...
    public async Task Sharing_a_group_list_shares_the_inventory_it_is_measured_against()
    {
        ...
        var member = await context.AddTaskListAsync("Shopping");
        var group = await context.AddGroupListAsync("Saturday", member.Id);
        await context.MeasureAgainstAsync(group, warehouse.Id);

        await context.ShareAsync(group.Id, ShareAccessLevel.ReadOnly);

        Assert.NotNull(await context.WarehouseShareRepository.FindExistingAsync(
            warehouse.Id, context.RecipientId, CancellationToken.None));
    }
    ...
    public async Task Accepting_the_offer_opens_the_whole_tree_at_once()
    {
        ...
        Assert.NotNull(await context.TaskListShareRepository.FindAcceptedGrantAsync(
            member.Id, context.RecipientId, CancellationToken.None));
        Assert.NotNull(await context.WarehouseShareRepository.FindAcceptedGrantAsync(
            warehouse.Id, context.RecipientId, CancellationToken.None));
    }
    ...
    private sealed class CascadeTestContext
    {
        ...
        public CascadeTestContext()
        {
            ...
            _publicSharedItemReader = new PublicSharedItemReader(
                ...
                _warehouseRepository, new InMemoryInventoryRepository(), _userRepository);
        }

        private TaskListShareCascade Cascade => new(
            _taskRepository, _warehouseRepository, TaskListShareRepository, WarehouseShareRepository);
        ...
        public async Task<Warehouse> AddWarehouseAsync(string name)
        {
            var warehouse = Warehouse.Create(OwnerId, name);
            await _warehouseRepository.AddAsync(warehouse, CancellationToken.None);
            return warehouse;
        }

        public async Task MeasureAgainstAsync(TaskList taskList, Guid warehouseId)
        {
        ...
        public async Task ClaimLinkToAsync(Guid taskListId)
        {
            ...
            await new ClaimPublicShareLinkCommandHandler(
                    ...
                    TaskListShareRepository, new InMemoryCalendarEventShareRepository(), WarehouseShareRepository,
