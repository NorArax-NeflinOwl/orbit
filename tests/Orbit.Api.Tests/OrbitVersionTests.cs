...
public sealed class OrbitVersionTests
{
    ...
    {
        Assert.Equal("ver:0.1.32+gitHash:51536f360a130d98b3b631da81dce22e38c0903a", AStampedBuild().Full);
    }
    ...
    /// <summary>
    /// An unnumbered build still knows which commit it is, and that is the whole point of the line:
    /// nobody is comparing "0.0.0-dev" against anything, they are asking which code is running. The SDK
    /// stamps the real HEAD next to its own "1.0.0" default, and this used to be thrown away with the
    /// number - so such a build showed no hash at all and the footer was not pressable, which is
    /// exactly the case the hash exists for.
    /// </summary>
    [Fact]
    public void A_build_nobody_numbered_still_says_which_commit_it_is()
    {
        ...
        // And it is pressable, because there is more of the hash to reveal.
        Assert.True(version.CanShowTheWholeCommit);
    }
    ...
    public void A_number_with_no_commit_behind_it_still_reads_as_a_version()
    {
        ...
        var version = new OrbitVersion("0.2.0", string.Empty);

        Assert.Equal("ver:0.2.0", version.Short);
        Assert.Equal("ver:0.2.0", version.Full);
        // Nothing offers the press: a number that looks pressable and then does nothing is worse than
    ...
    {
        var version = new OrbitVersion("0.1.1", "abc");
    ...
    private static OrbitVersion AStampedBuild()
        => new("0.1.32", "51536f360a130d98b3b631da81dce22e38c0903a");
