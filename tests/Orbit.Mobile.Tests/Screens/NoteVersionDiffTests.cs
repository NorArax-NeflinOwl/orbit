...
/// <summary>
/// What the review window shows between two versions of a note. Small on its own, but it is the whole
/// basis on which somebody decides which version to keep - a diff that overstates the change makes
/// every review look like a conflict, and one that understates it loses somebody's work quietly.
/// </summary>
public sealed class NoteVersionDiffTests
{
    [Fact]
    public void Two_versions_that_say_the_same_thing_show_nothing_changed()
    {
        ...
        var diff = NoteVersionDiff.Between(before, Lines("milk", "bread"));

        Assert.All(diff, line => Assert.Equal(LineChange.Unchanged, line.Change));
        Assert.False(NoteVersionDiff.Differ(before, Lines("milk", "bread")));
    }

    [Fact]
    public void An_added_line_is_marked_and_nothing_else_is()
    {
        var diff = NoteVersionDiff.Between(Lines("milk"), Lines("milk", "bread"));
    ...
    {
        var diff = NoteVersionDiff.Between(Lines("milk", "bread"), Lines("eggs", "milk", "bread"));
    ...
    public void A_removed_line_is_shown_as_removed_rather_than_left_out()
    {
        ...
        var diff = NoteVersionDiff.Between(Lines("milk", "bread"), Lines("bread"));

        Assert.Equal([("milk", LineChange.Removed), ("bread", LineChange.Unchanged)], Described(diff));
    }

    /// <summary>Ticking something is the change somebody made; a diff blind to it shows an empty review.</summary>
    [Fact]
    public void Ticking_a_line_counts_as_changing_it()
    {
        var before = new[] { new NoteContentLineDto("milk", true, false) };
        var after = new[] { new NoteContentLineDto("milk", true, true) };

        Assert.True(NoteVersionDiff.Differ(before, after));
    }

    [Fact]
    public void An_edited_line_reads_as_the_old_one_out_and_the_new_one_in()
    {
        var diff = NoteVersionDiff.Between(Lines("milk"), Lines("oat milk"));

        Assert.Equal([("milk", LineChange.Removed), ("oat milk", LineChange.Added)], Described(diff));
    }

    private static IReadOnlyList<NoteContentLineDto> Lines(params string[] texts)
        => [.. texts.Select(text => new NoteContentLineDto(text, false, false))];
