...
public sealed class CalendarEventShareRepository : ICalendarEventShareRepository
{
    ...
    public async Task<IReadOnlyList<CalendarEventShare>> GetSharesToAsync(Guid ownerUserId, Guid recipientUserId, CancellationToken cancellationToken)
    {
        var entities = await _dbContext.CalendarEventShares
            .AsNoTracking()
    ...
    {
        var removed = await _dbContext.CalendarEventShares
