...
public sealed class TaskListShareRepository : ITaskListShareRepository
{
    ...
    public async Task<IReadOnlyList<TaskListShare>> GetSharesToAsync(Guid ownerUserId, Guid recipientUserId, CancellationToken cancellationToken)
    {
        var entities = await _dbContext.TaskShares
            .AsNoTracking()
    ...
    {
        var removed = await _dbContext.TaskShares
