...
{
    /// <summary>
    /// Where Google is told to send the reader back to, which each platform spells differently: Android
    /// redirects to the application id's own scheme, iOS to the client id with its dot-separated parts
    /// reversed. Both are registered with Google against this app, and neither can be built without
    /// knowing which client is asking - hence the argument rather than a property.
    /// </summary>
    Uri CallbackAddressFor(string clientId);
...
public sealed class GoogleSignIn
{
    ...
    internal Uri BuildAuthorizationAddress(string clientId, string codeVerifier)
    {
        ...
        query["redirect_uri"] = _browser.CallbackAddressFor(clientId).ToString();
        ...
        string clientId, string code, string codeVerifier, CancellationToken cancellationToken)
    {
        var response = await _httpClient.PostAsync(
            ...
            new FormUrlEncodedContent(new Dictionary<string, string>
            {
                ["code"] = code,
                ["client_id"] = clientId,
                ["redirect_uri"] = _browser.CallbackAddressFor(clientId).ToString(),
