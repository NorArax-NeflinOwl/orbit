...
public sealed class LocalInventoryRepository : ICopyReviewStore
{
    ...
    /// <summary>
    /// Puts this shelf away, or brings it back - see Orbit.Core.Folders.BuiltInFolder.Archived. Queued
    ...
    {
        await using var dbContext = await _dbContextFactory.CreateDbContextAsync(cancellationToken);
        if (await dbContext.Inventories.FirstOrDefaultAsync(candidate => candidate.LocalId == localId, cancellationToken) is not { } inventory)
        {
            return LocalWriteOutcome.NotFound;
        }
        ...
        inventory.IsArchived = isArchived;
        ...
        if (inventory.ServerId is not null)
        {
            Enqueue(dbContext, localId, OutboxOperation.Archive, _timeProvider.GetUtcNow(), inventory.ServerId);
        }
        else if (!CopiesForEditing.IsAwaitingReview(inventory))
        {
            await LostCreates.QueueAgainAsync(
