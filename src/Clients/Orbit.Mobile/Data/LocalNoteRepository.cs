...
public sealed class LocalNoteRepository : ICopyReviewStore
{
    ...
    /// <summary>
    /// Puts this note in a folder, or takes it out of one. Queued as its own kind of change, because
    ...
    /// changes where a note is kept, not what it says, and a note that jumped to the top of the list
    /// for having been tidied away would read as having been edited.
    ///
    /// A note nobody may change here is not filed either: filing is the owner's decision about their
    ...
    /// <summary>
    /// Puts this note away, or brings it back - see Orbit.Core.Folders.BuiltInFolder.Archived. Queued
        ...
        Guid localId, bool isArchived, CancellationToken cancellationToken = default)
    {
        ...
        if (note.ServerId is not null)
        {
            Enqueue(dbContext, localId, OutboxOperation.Archive, _timeProvider.GetUtcNow(), note.ServerId);
        }
        else if (!CopiesForEditing.IsAwaitingReview(note))
        {
            await LostCreates.QueueAgainAsync(
