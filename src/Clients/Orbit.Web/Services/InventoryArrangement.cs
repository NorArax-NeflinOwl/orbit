...
/// <summary>
/// The order this reader put their inventories in.
///
/// Kept on the device rather than on the account, like <see cref="TaskListArrangement"/>: it describes
/// how one person reads one page and says nothing about the inventories themselves - somebody a shelf is
/// shared with has their own idea of which one matters most, and neither answer is the right one to
/// impose on the other.
/// </summary>
public sealed class InventoryArrangement
{
    private const string OrderKey = "orbit-warehouse-order";

    private readonly IJSRuntime _jsRuntime;

    public InventoryArrangement(IJSRuntime jsRuntime)
    {
        _jsRuntime = jsRuntime;
    }

    /// <summary>
    /// The ids the reader has arranged, first to last. An inventory that is not on it - one created or
    ...
    public Task SetOrderAsync(IReadOnlyList<Guid> orderedInventoryIds)
    {
        Order = orderedInventoryIds;
        return WriteAsync(OrderKey, JsonSerializer.Serialize(orderedInventoryIds));
    }

    /// <summary>
    /// The inventories in the arranged order, with anything unplaced after them in the order it arrived.
    /// </summary>
    public IEnumerable<T> Arrange<T>(IEnumerable<T> inventories, Func<T, Guid> idOf)
    {
        var placed = Order
            .Select((id, index) => (id, index))
            .ToDictionary(entry => entry.id, entry => entry.index);

        return inventories.OrderBy(inventory => placed.TryGetValue(idOf(inventory), out var index) ? index : int.MaxValue);
    }
