...
/// <summary>
/// How a warehouse's restock list is built and when it comes round. Kept beside the tracking of which
/// list a warehouse owns (see <see cref="IInventoryManagedTaskListRepository"/>), because it is settings
/// for that list rather than for the warehouse: a warehouse with no restock list has nothing for these
/// to mean.
/// </summary>
...
public sealed record RestockListSettings(bool OnlyLinkedWithDueDate, TimeOnly RefreshTimeOfDay)
{
    ...
    /// <summary>What a warehouse has before anybody changes anything - the rule Orbit has always used.</summary>
