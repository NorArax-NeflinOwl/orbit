...
/// <summary>
/// What Orbit calls the list it keeps for an inventory, and the entries on it. One place, because two
/// paths create restock entries (an inventory item going low, and a task list coming up short) and both
...
public static class RestockTaskNaming
{
    ...
    /// <summary>
    /// The list's title, which names the inventory it restocks: an account with three inventories had
    /// three lists called the same thing, and no way to tell from the tasks page which was which.
    /// </summary>
    public static string TitleFor(string inventoryName)
        => string.IsNullOrWhiteSpace(inventoryName) ? ListTitlePrefix : $"{ListTitlePrefix} - {inventoryName.Trim()}";

    /// <summary>Whether a title is one of ours, so an existing list can be renamed when its inventory is.</summary>
    ...
    /// is meant to hold - reading the errand should not need the inventory open beside it, and "5" of
