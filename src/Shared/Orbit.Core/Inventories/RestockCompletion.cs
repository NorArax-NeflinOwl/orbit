...
public sealed class RestockCompletion
{
    ...
    /// <summary>
    /// Settles every finished errand on <paramref name="taskListId"/> against the inventory behind it:
    ...
    {
        if (await _managedTaskListRepository.GetInventoryIdAsync(taskListId, cancellationToken) is not { } inventoryId)
        {
        ...
        foreach (var errand in finished)
        {
            ...
            {
                // The product is gone from the inventory. The errand is still finished - there is
    ...
    /// <summary>
    /// Brings every item in the inventory behind <paramref name="taskListId"/> up to its minimum - the
