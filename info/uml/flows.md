...
folder that has gone, with nothing to carry it. And they are pushed **first**, ahead of everything that
can be filed into one (notes, task lists, events and shelves), because something filed into a folder
made offline names a folder the server has not been told about yet. That filing does not fail and it is not lost: `FolderNotOnTheServerYet` stops the send with
