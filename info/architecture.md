...
(`dotnet test Orbit.CI.slnf`) on `ubuntu-latest` with .NET SDK 10, then runs the five harnesses covering
...
(a registered service worker receiving real push events, and the Notification and Push APIs),
`ci/verify-note-surface.mjs` for `wwwroot/js/checklistTextEditor.js` (a note drawn on the writing surface
and read back off it, which is what a rule across a note was silently lost by - see issue #294), and
`ci/verify-menu-anchor.mjs` for `wwwroot/js/menuAnchor.js` (where a fixed panel lands, which is
`getBoundingClientRect` arithmetic bUnit measures as zero), and `ci/verify-map-markers.mjs` for
`updateLocations` in `wwwroot/js/locationMap.js` (the map's Refresh, against a real Leaflet map - see
issue #293). Every later job depends on this one, so a failure here stops the deploy before an image is
built.
