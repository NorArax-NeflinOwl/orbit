...
- ~~**The counts on folder entries.** Notes and Tasks list their folders in the menu with the number of
  ...
  is empty (2026-09-24; `ScreenMenuEntry.Note`, and `info/functionality.md` on why only an empty one
  goes). Filing is under the note's or the list's own name once it is open, the lists having
