...
| `Permissions__Secret` | Set by the deploy workflow itself, not by hand - a fresh random value on every release, from which the four permission unlock codes are derived. Missing means the API generates one per restart, so codes change whenever a revision restarts. | The "Generate this release's permission unlock codes" step in [main_orbit.yml](../.github/workflows/main_orbit.yml), which also prints the codes in the run summary. Deliberately absent from the required-variables check, which lists only what a human has to set. |
