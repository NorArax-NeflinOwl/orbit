...
| `Permissions__Secret` | Set by the deploy workflow itself, not by hand - a fresh random value on every release, from which the four permission unlock codes are derived. Missing means the API generates one per restart, so codes change whenever a revision restarts. | Container App secret (`permission-secret`), written by the "Deploy orbit-api" step in [main_orbit.yml](../.github/workflows/main_orbit.yml); the codes are printed in that run's summary and exist nowhere else, since a Container App secret cannot be read back. Deliberately absent from the required-variables check, which lists only what a human has to set. |
